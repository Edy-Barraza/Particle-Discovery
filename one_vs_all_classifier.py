...
# <h2>Training Characteristics</h2>
...
mod,_,_, _,loss_list,acc_list = train_model(model_conv, criterion, optimizer_conv, num_epochs=3)
...
mod,pred_list,label_list, prob_list,loss_list2,acc_list2 = train_model(mod, criterion, optimizer_conv, num_epochs=15)
...
plt.plot(fpr, tpr, color='blue',
         lw=lw, label=class_names[0]+'ROC curve (area = %0.2f)' % roc_auc)
plt.plot([0, 1], [0, 1], color='red', lw=lw, linestyle='--')
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title(class_names[0]+'Receiver Operating Characteristic')
